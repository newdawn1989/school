...
int main(int argc, char *argv[])
{
    ...
    int rotate_points = 5+((rotate_maxmean-1.0)*18.75);
    int smooth_points = 5+((smooth_maxmean-1.0)*2.64);
    
    if(rotate_points > 28) rotate_points = 28;
    if(smooth_points > 28) smooth_points = 28;
    int total_points = rotate_points+smooth_points;
    if(total_points > 50) total_points = 50;
    
    if (autograder) {
	printf("\nbestscores:%.1f:%.1f:\n", rotate_maxmean, smooth_maxmean);
    printf("\tRotate Points: %d\n\tSmooth Points: %d\n\tTotal Points: %d\n",rotate_points, smooth_points, total_points);
    }
    else {
